...
const CryptogramGame: React.FC<CryptogramGameProps> = ({ onFinish, onClose }) => {
  ...
  useEffect(() => {
    ...
    if (!isFinished && score > 10 && gameData) {
      timer = setInterval(() => {
        setScore(prev => Math.max(10, prev - 1));
      }, 1000);
    }
    return () => clearInterval(timer);
  }, [isFinished, score, gameData]);
  ...
  return (
    <div className="fixed inset-0 z-50 bg-stone-50 dark:bg-zinc-950 overflow-y-auto">
      <div className="min-h-screen p-4 md:p-8 flex flex-col max-w-6xl mx-auto">
        <div className="bg-white dark:bg-zinc-900 rounded-[2.5rem] p-6 md:p-10 shadow-2xl border border-stone-200 dark:border-zinc-800 flex-1 flex flex-col">
          ...
          <div className="flex-1 relative border-2 border-stone-200 dark:border-zinc-700 rounded-2xl overflow-hidden bg-stone-50 dark:bg-zinc-900/50 mb-8">
