...
interface EvangelismSearchPageProps {
  initialQuery?: string;
}

export default function EvangelismSearchPage({ initialQuery = '' }: EvangelismSearchPageProps) {
  const { user } = useAuth();
  ...
  const handleSearch = async (overrideQuery?: string) => {
    ...
    setAudioUrl(null);
    try {
      const prompt = `Pesquise o termo relacionado ao evangelismo e missões "${searchTarget}" agregando insights de grandes evangelistas como Billy Graham, Reinhard Bonnke, T.L. Osborn, e obras clássicas sobre missões e evangelismo.
      Consulte também:
      - "A Evangelização e a Soberania de Deus" de J.I. Packer
      - "O Plano Mestre de Evangelismo" de Robert Coleman
      - "Missão Integral" de René Padilla
      
      Estruture os resultados para oferecer uma visão prática e bíblica sobre como aplicar este conceito no evangelismo moderno, missões urbanas e transculturais. Cite fontes e versículos relevantes.`;
      const response = await geminiService.generateText(prompt, "Você é um PhD em Missiologia e Evangelismo.");
      setResult(response);
      ...
      console.error(error);
      showToast("Erro ao realizar a busca de evangelismo.", 'error');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleSendToCollection = async () => {
    ...
    try {
      await addDoc(collection(db, 'audioCollection'), {
        userId: user.id,
        title: `Pesquisa Evangelismo: ${query}`,
        text: result,
        audioUrl: audioUrl,
        createdAt: new Date().toISOString(),
        type: 'evangelism_search'
      });
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8">
      <SearchLoadingOverlay isVisible={isLoading || isGeneratingSpeech} message={isLoading ? "Pesquisando Evangelismo..." : "Gerando Áudio..."} />
        ...
        <div className="flex items-center gap-4 mb-8">
          ...
          <div>
            <div className="flex items-center gap-3">
              <h2 className="text-2xl font-bold font-display">Busca de Evangelismo</h2>
            </div>
            <p className="text-stone-500 text-sm">Pesquise termos, estratégias e conceitos missionários.</p>
          </div>
        </div>

        <div className="relative group">
          <Search className="absolute left-6 top-1/2 -translate-y-1/2 text-stone-400 group-focus-within:text-orange-600 transition-colors" size={24} />
          <input 
            type="text"
            placeholder="🔥 Ex: Plano de Salvação, Janela 10/40, Evangelismo Criativo..."
            value={query}
            onChange={(e) => setQuery(e.target.value)}
            onKeyDown={(e) => e.key === 'Enter' && handleSearch()}
            className="w-full pl-16 pr-48 py-6 bg-stone-50 dark:bg-zinc-800/50 border border-stone-200 dark:border-zinc-700 rounded-3xl outline-none focus:ring-4 focus:ring-orange-500/10 focus:border-orange-500 transition-all text-lg"
          />
          <div className="absolute right-4 top-1/2 -translate-y-1/2 flex items-center gap-2">
            ...
            <button 
              ...
              className="px-6 py-3 bg-orange-600 text-white font-bold rounded-2xl hover:bg-orange-700 transition-all disabled:opacity-50 flex items-center gap-2"
            >
          ...
          >
            <div className="flex items-center justify-between mb-8 pb-4 border-b border-stone-100 dark:border-zinc-800">
              <div className="flex items-center gap-4">
                ...
                <button
                  ...
                  className="p-2 bg-orange-50 dark:bg-orange-900/20 text-orange-600 rounded-xl hover:bg-orange-100 transition-colors disabled:opacity-50"
              ...
              <div className="flex items-center gap-2">
                {audioUrl && (
                  <button 
                    ...
                    className="px-4 py-2 bg-orange-600 text-white text-xs font-bold rounded-xl hover:bg-orange-700 transition-colors flex items-center gap-2"
                  >
                    <Download size={14} />
            ...
            {audioUrl && (
              <div className="mb-8 bg-stone-50 dark:bg-zinc-800/50 p-6 rounded-3xl space-y-4">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 bg-orange-600 text-white rounded-full flex items-center justify-center animate-pulse">
                    <Volume2 size={20} />
                  </div>
                  <div>
      ...
      {!result && !isLoading && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {[
            { title: 'Plano de Salvação', desc: 'Aprenda a apresentar o evangelho de forma clara.', icon: <Sparkles size={20} /> },
            { title: 'Missões Urbanas', desc: 'Estratégias para alcançar as grandes cidades.', icon: <Book size={20} /> },
            { title: 'Janela 10/40', desc: 'Entenda os maiores desafios missionários globais.', icon: <BookOpen size={20} /> },
          ].map((item, i) => (
            <div key={i} className="p-8 bg-white dark:bg-zinc-900 rounded-[2.5rem] border border-stone-100 dark:border-zinc-800 shadow-sm hover:shadow-md transition-all">
              <div className="p-3 bg-orange-50 dark:bg-orange-900/20 text-orange-600 rounded-xl w-fit mb-4">
                {item.icon}
              </div>
